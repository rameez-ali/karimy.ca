        ...
        <div class="col-12 col-md-10 d-none d-xl-block">
            <nav class="site-navigation position-relative text-right" role="navigation">
                ...
                <ul class="site-menu js-clone-nav mr-auto d-none d-lg-block pl-4">
                    <li><a href="{{ route('page.home') }}" class="{{ (Route::currentRouteName() === "page.home") ? 'active' : 'disable' }}"  >{{ __('frontend.header.home') }}
                    <!--@if(Route::currentRouteName() === 'page.home')-->
                    <!--    // code-->
                    <!--    demo-->
                    <!--@endif-->
                    <!--{{ Route::currentRouteName() }}-->
                    <!--{{ (Route::currentRouteName() === "page.home") ? 'active' : 'disable' }}-->
                    </a></li>
                    <li><a href="{{ route('page.categories') }}" class="{{ (Route::currentRouteName() === "page.categories") ? 'active' : 'disable' }}" >{{ __('frontend.header.listings') }}</a></li>
                    ...
                    @guest
                        <li class="ml-xl-3 login"><a href="{{ route('login') }}" class="{{ (Route::currentRouteName() === "login") ? 'active' : 'disable' }}" ><span class="border-left pl-xl-4"></span>{{ __('frontend.header.login') }}</a></li>
                        @if (Route::has('register'))
                            <li><a href="{{ route('register') }}" class="{{ (Route::currentRouteName() === "register") ? 'active' : 'disable' }}">{{ __('frontend.header.register') }}</a></li>
                        @endif
                    @else
                        <li class="has-children">
                            ...
                            <ul class="dropdown">
                                ...
                                @else
                                     <a href="{{ route('page.pricing') }}">{{ __('frontend.header.dashboard') }}</a>
                                @endif
                                    @endif
                                </li>

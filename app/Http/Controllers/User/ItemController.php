...
class ItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach($select_categories as $select_categories_key => $select_category)
        {
         ...
         $terms_condition = $request->terms_condition ? $request->terms_condition : 0;
         $authorize_canada = $request->authorize_canada ? $request->authorize_canada : 0;
         $registered_business_canada = $request->registered_business_canada ? $request->registered_business_canada : 0;
         $above_18 = $request->above_18 ? $request->above_18 : 0;
        ...
        $new_item = new Item(array(
            ...
            'authorize_canada'=>$authorize_canada,
            'registered_business_canada'=>$registered_business_canada,
            'above_18'=>$above_18
        ));
        $new_item->save();
        ...
        //Here we will add email function to user for adding new listing
     ...
     if($settings->settings_site_smtp_enabled == Setting::SITE_SMTP_ENABLED)
        ...
        $email_subject = __('email.add_new_listing.subject');
        
        $email_notify_message = [
            __('email.add_new_listing.body.body-1'),
            __('email.add_new_listing.body.body-3')
        ];
        
        $routeURL = Auth::user()->hasPaidSubscription() ? route('user.index') : route('page.pricing');
         try
        {
            // to user
        ...
        //End email function to user for adding new listing
    ...
    public function update(Request $request, Item $item)
    {
        ...
        $terms_condition = $request->terms_condition ? $request->terms_condition : 1;
        
        $authorize_canada = $request->authorize_canada ? $request->authorize_canada : 1;
          
        $registered_business_canada = $request->registered_business_canada ? $request->registered_business_canada : 1;
            
        $above_18 = $request->above_18 ? $request->above_18 : 1;
        ...
        $item->terms_condition = $terms_condition;
        $item->authorize_canada = $authorize_canada;
        $item->registered_business_canada = $registered_business_canada;
        $item->above_18 = $above_18;
